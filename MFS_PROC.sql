CREATE OR REPLACE procedure REPORT_USER.IT_MIS_MFS_DAILY_REPORT_PROC  is
...
AND to_char(to_date(a.name,'dd/mm/yyyy'),'MONTH')=b.FULL_NAMES_FRENCH
...
select t.*,STATUT,VILLE,COMMUNES,GOUVERNORATS,REGION_NATURELLE,SITE_TYPE,to_char(sysdate-1,'yyyy-mm-dd') REPORT_DATE
    from IT_MIS_MOMO_NEW_ACTIVATION@MIS2MOMOREPORT t,IT_MIS_GLOBAL_MJR_CELLID_RGS90 a,IT_MIS_CELLID_FINAL_2013 b,
...
from (
     ...
     ) t,IT_MIS_GLOBAL_MJR_CELLID_RGS90 a,IT_MIS_CELLID_FINAL_2013 b
where '00'||t.msisdn = a.MSISDN(+)
and a.MAJOR_CELLID=b.cellid(+) 
...
select t.*,STATUT,VILLE,COMMUNES,GOUVERNORATS,REGION_NATURELLE,SITE_TYPE,to_char(sysdate-1,'yyyy-mm-dd') REPORT_DATE
    from IT_MIS_MOMO_NEW_ACTIVATIONP@MIS2MOMOREPORT t,IT_MIS_GLOBAL_MJR_CELLID_RGS90 a,IT_MIS_CELLID_FINAL_2013 b,
    (select SUBNO,'TRANSACTED' STATUT from(
                    select frommsisdn SUBNO
                    from EWP_FINANCIAL_CSV@MIS2MOMOREPORT
                    where TRANSACTIONTYPE in ('CASH_OUT','DEBIT','EXTERNAL_PAYMENT','PAYMENT')
...
select TRANSACTIONTYPE,'RGS30_MOC_MTC' as RGS_TYPE,  to_date(to_char(sysdate-1,'yyyy-mm-dd'),'yyyy-mm-dd') || ' ---> '||to_date(to_char(sysdate-1,'yyyy-mm-dd'),'yyyy-mm-dd') Period, count(distinct msisdn) total_msisdn, to_date(to_char(sysdate-1,'yyyy-mm-dd'),'yyyy-mm-dd') report_date
from( 
...
where substr(DATETIME,1,10) between to_char(sysdate-1,'yyyy-mm-dd') and to_char(sysdate-1,'yyyy-mm-dd')
...
CREATE TABLE IT_MIS_GLOBAL_MJR_CELLID_RGS90 AS
...
FROM  IT_MIS_GLOBAL_MJR_CELLID_RGS90 a,IT_MIS_CELLID_FINAL_2013 b
...
FROM  IT_MIS_GLOBAL_MJR_CELLID_RGS90 a
